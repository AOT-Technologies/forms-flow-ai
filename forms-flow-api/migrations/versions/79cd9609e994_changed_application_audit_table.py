...
def upgrade():
    ...
    op.add_column('application_audit', sa.Column('form_uri', sa.String(length=100), nullable=False))
...
def downgrade():
    ...
    op.add_column('application_audit', sa.Column('application_name', sa.VARCHAR(length=100), autoincrement=False, nullable=False))
    op.alter_column('application_audit', 'application_id',
               existing_type=sa.INTEGER(),
               nullable=True)
    op.drop_column('application_audit', 'form_uri')
