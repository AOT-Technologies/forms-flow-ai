# flake8: noqa: D205,D400,D205,D201
"""
...
ATOM_XML_ENTRY_TYPE_P = re.compile("^application/atom\+xml.*type.*entry")  # noqa: W605
ATOM_XML_FEED_TYPE = "application/atom+xml;type=feed"
ATOM_XML_FEED_TYPE_P = re.compile("^application/atom\+xml.*type.*feed")  # noqa: W605
CMIS_TREE_TYPE = "application/cmistree+xml"
CMIS_TREE_TYPE_P = re.compile("^application/cmistree\+xml")  # noqa: W605
...
class AtomPubBinding(Binding):
    ...
    def delete(self, url, username, password, **kwargs):
        ...
        if type(url) == bytes:
            url = url.decode("utf8")
        resp = Rest().delete(url, username=username, password=password, **kwargs)
    ...
    def post(self, url, username, password, payload, contentType, **kwargs):
        ...
        if type(url) == bytes:
            url = url.decode("utf8")
        resp = Rest().post(
            url, payload, contentType, username=username, password=password, **kwargs
        )
        if resp.status_code == 200:
            try:
                return minidom.parseString(resp.content)
...
class RepositoryService(RepositoryServiceIfc):
    ...
    def reload(self, obj):
        ...
        obj.xmlDoc = obj._cmisClient.binding.get(
            obj._cmisClient.repositoryUrl.encode("utf-8"),
    ...
    def getRepository(self, client, repositoryId):
        ...
        doc = client.binding.get(
            client.repositoryUrl, client.username, client.password, **client.extArgs
        )
    ...
    def getDefaultRepository(self, client):
        ...
        repository = AtomPubRepository(
            client, [e for e in workspaceElements if e.nodeType == e.ELEMENT_NODE][0]
        )
        return repository
...
class AtomPubCmisObject(CmisObject):
    ...
    def getAllowableActions(self):
        ...
        if self._allowableActions == {}:
            ...
            allowElements = self.xmlDoc.getElementsByTagNameNS(
                CMIS_NS, "allowableActions"
            )
            assert (
                len(allowElements) == 1
            ), "Expected response to have exactly one allowableActions element"
            allowElement = allowElements[0]
            for node in [
                e for e in allowElement.childNodes if e.nodeType == e.ELEMENT_NODE
            ]:
    ...
    def getProperties(self):
        ...
        if self._properties == {}:
            if self.xmlDoc is None:
                self.reload()
            propertiesElement = self.xmlDoc.getElementsByTagNameNS(
                CMIS_NS, "properties"
            )[0]
    ...
    def updateProperties(self, properties):
        ...
        args = {}
        if (
            "cmis:changeToken" in self.properties.keys()
            and self.properties.get("cmis:changeToken") is not None
        ):
        ...
        objectTypeId = None
        if (
            "cmis:objectTypeId" in self.properties.keys()
            and "cmis:objectTypeId" not in properties.keys()
        ):
...
class AtomPubRepository(object):
    ...
    def reload(self):
        ...
        self.logger.debug("Reload called on object")
        self.xmlDoc = self._cmisClient.binding.get(
            self._cmisClient.repositoryUrl.encode("utf-8"),
            self._cmisClient.username,
            self._cmisClient.password,
        )
        self._initData()
    ...
    def getSupportedPermissions(self):
        ...
        if not self._permissions:
            ...
            assert (
                len(suppEls) == 1
            ), "Expected the repository service document to have one element named supportedPermissions"
    ...
    def getPermissionDefinitions(self):
        ...
        if self._permDefs == {}:
            ...
            assert (
                len(aclEls) == 1
            ), "Expected the repository service document to have one element named aclCapability"
            aclEl = aclEls[0]
            perms = {}
            for e in aclEl.childNodes:
                if e.localName == "permissions":
                    ...
                    assert (
                        len(permEls) == 1
                    ), "Expected permissions element to have a child named permission"
                    descEls = e.getElementsByTagNameNS(CMIS_NS, "description")
                    assert (
                        len(descEls) == 1
                    ), "Expected permissions element to have a child named description"
        ...
        if self._permMap == {}:
            ...
            for e in aclEl.childNodes:
                ...
                if e.localName == "mapping":
                    ...
                    assert (
                        len(permEls) >= 1
                    ), "Expected mapping element to have at least one permission element"
    ...
    def getPropagation(self):
        ...
        if not self._propagation:
            ...
            assert (
                len(propEls) == 1
            ), "Expected the repository service document to have one element named propagation"
    ...
    def getRepositoryId(self):
        ...
        if self._repositoryId is None:
            if self.xmlDoc is None:
                self.reload()
            self._repositoryId = self.xmlDoc.getElementsByTagNameNS(
                CMIS_NS, "repositoryId"
            )[0].firstChild.data
    ...
    def getRepositoryName(self):
        ...
        if self._repositoryName is None:
            if self.xmlDoc is None:
                self.reload()
            if self.xmlDoc.getElementsByTagNameNS(CMIS_NS, "repositoryName")[
                0
            ].firstChild:
                self._repositoryName = self.xmlDoc.getElementsByTagNameNS(
                    CMIS_NS, "repositoryName"
                )[0].firstChild.data
            else:
    ...
    def getRepositoryInfo(self):
        ...
        if not self._repositoryInfo:
            if self.xmlDoc is None:
                self.reload()
            repoInfoElement = self.xmlDoc.getElementsByTagNameNS(
                CMISRA_NS, "repositoryInfo"
            )[0]
            for node in repoInfoElement.childNodes:
                if (
                    node.nodeType == node.ELEMENT_NODE
                    and node.localName != "capabilities"
                    and node.localName != "aclCapability"
                ):
                    try:
    ...
    def getCapabilities(self):
        ...
        if not self._capabilities:
            if self.xmlDoc is None:
                self.reload()
            capabilitiesElement = self.xmlDoc.getElementsByTagNameNS(
                CMIS_NS, "capabilities"
            )[0]
            for node in [
                e
                for e in capabilitiesElement.childNodes
    ...
    def getTypeDefinitions(self, **kwargs):
        ...
        typesXmlDoc = self._cmisClient.binding.get(
            typesUrl, self._cmisClient.username, self._cmisClient.password, **kwargs
        )
        entryElements = typesXmlDoc.getElementsByTagNameNS(ATOM_NS, "entry")
        types = []
        for entryElement in entryElements:
    ...
    def getObject(self, objectId, **kwargs):
        ...
        return getSpecializedObject(
            AtomPubCmisObject(self._cmisClient, self, CmisId(objectId), **kwargs),
            **kwargs,
        )
    ...
    def getObjectByPath(self, path, **kwargs):
        ...
        assert len(entryElements) == 1, (
            "Expected entry element in result from calling %s" % byObjectPathUrl
        )
        return getSpecializedObject(
            AtomPubCmisObject(
                self._cmisClient, self, xmlDoc=entryElements[0], **kwargs
            ),
            **kwargs,
        )
    ...
    def query(self, statement, **kwargs):
        ...
        your select statement, or just use "SELECT \*". # noqa: W605
    ...
    def createDocumentFromString(
        ...
        return parentFolder.createDocument(
            name, properties, io.StringIO(contentString), contentType, contentEncoding
        )
    ...
    def createDocument(
        ...
        xmlDoc = getEntryXmlDoc(
            self, None, properties, contentFile, contentType, contentEncoding
        )
    ...
    def getUriTemplates(self):
        ...
        if self._uriTemplates == {}:
            ...
            uriTemplateElements = self.xmlDoc.getElementsByTagNameNS(
                CMISRA_NS, "uritemplate"
            )
            ...
            for uriTemplateElement in uriTemplateElements:
                ...
                for node in [
                    e
                    for e in uriTemplateElement.childNodes
                    if e.nodeType == e.ELEMENT_NODE
                ]:
                ...
                self._uriTemplates[templType] = UriTemplate(
                    template, templType, mediatype
                )
    ...
    def getCollection(self, collectionType, **kwargs):
        ...
        result = self._cmisClient.binding.get(
            self.getCollectionLink(collectionType).encode("utf-8"),
            self._cmisClient.username,
            self._cmisClient.password,
            **kwargs,
        )
    ...
    def getCollectionLink(self, collectionType):
        ...
        for collectionElement in collectionElements:
            ...
            for node in [
                e for e in collectionElement.childNodes if e.nodeType == e.ELEMENT_NODE
            ]:
        ...
        if self._xmlDoc:
            ...
            for entryElement in entryElements:
                cmisObject = getSpecializedObject(
                    AtomPubCmisObject(
                        self._cmisClient, self._repository, xmlDoc=entryElement
                    )
                )
...
class AtomPubDocument(AtomPubCmisObject):
    ...
    def isCheckedOut(self):
        ...
        return parseBoolValue(
            self.getProperties().get("cmis:isVersionSeriesCheckedOut")
        )
    ...
    def getContentStream(self):
        ...
        assert (
            len(contentElements) == 1
        ), "Expected to find exactly one atom:content element."
    ...
    def setContentStream(self, contentFile, contentType=None):
        ...
        if (
            "cmis:changeToken" in self.properties.keys()
            and self.properties["cmis:changeToken"] is not None
        ):
            self.logger.debug("Change token present, adding it to args")
            args = {"changeToken": self.properties["cmis:changeToken"]}
...
class AtomPubFolder(AtomPubCmisObject):
    ...
    def createDocumentFromString(
        ...
        return self._repository.createDocumentFromString(
            name, properties, self, contentString, contentType, contentEncoding
        )
...
class AtomPubObjectType(ObjectType):
    ...
    def isIncludedInSupertypeQuery(self):
        ...
        return parseBoolValue(
            self._getElementValue(CMIS_NS, "includedInSupertypeQuery")
        )
    ...
    def getLink(self, rel, linkType):
        ...
        for linkElement in linkElements:
            ...
            if (
                "rel" in linkElement.attributes.keys()
                and "type" in linkElement.attributes.keys()
            ):
                relAttr = linkElement.attributes["rel"].value
                typeAttr = linkElement.attributes["type"].value
        ...
        if not self._entries:
            ...
            if principalId in self._entries.keys():
                if access not in self._entries[principalId].permissions:
                    ...
                    if not self._entries:
                        self._entries = {
                            principalId: AtomPubACE(principalId, perms, direct)
                        }
                    else:
                        self._entries[principalId] = AtomPubACE(
                            principalId, perms, direct
                        )
            else:
                self._entries[ace.principalId] = ace
...
class AtomPubChangeEntry(ChangeEntry):
    ...
    def getId(self):
        ...
        if self._changeEntryId is None:
            self._changeEntryId = self._xmlDoc.getElementsByTagNameNS(ATOM_NS, "id")[
                0
            ].firstChild.data
    ...
    def getChangeType(self):
        ...
        if self._changeType is None:
            self._changeType = self._xmlDoc.getElementsByTagNameNS(
                CMIS_NS, "changeType"
            )[0].firstChild.data
    ...
    def getChangeTime(self):
        ...
        if self._changeTime is None:
            self._changeTime = self._xmlDoc.getElementsByTagNameNS(
                CMIS_NS, "changeTime"
            )[0].firstChild.data
    ...
    def getProperties(self):
        ...
        if self._properties == {}:
            propertiesElement = self._xmlDoc.getElementsByTagNameNS(
                CMIS_NS, "properties"
            )[0]
            for node in [
                e for e in propertiesElement.childNodes if e.nodeType == e.ELEMENT_NODE
            ]:
                propertyName = node.attributes["propertyDefinitionId"].value
                if (
                    node.childNodes
                    ...
                    propertyValue = parsePropValue(
                        node.getElementsByTagNameNS(CMIS_NS, "value")[0]
                        .childNodes[0]
                        .data,
                        node.localName,
                    )
...
class AtomPubChangeEntryResultSet(AtomPubResultSet):
    ...
    def getResults(self):
        ...
        if self._xmlDoc:
            ...
            entries = []
            for entryElement in entryElements:
                changeEntry = AtomPubChangeEntry(
                    self._cmisClient, self._repository, entryElement
                )
