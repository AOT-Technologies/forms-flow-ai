...
class TestApplicationAnonymousResourcesByIds:
    def test_application_valid_post(self, client):
        ...
        rv = client.post("/form", headers=headers, json=get_form_request_anonymous_payload())
        assert rv.status_code == 201
        form_id = rv.json.get("formId")
        response = client.post("/public/application/create",json=get_application_create_payload(form_id))
    ...
    def test_application_invalid_post(self, client):
        ...
        response = client.post("/public/application/create",json=get_application_create_payload("asgdgasjg"))
    ...
    def test_application_unauthorized_post(self,client):
        ...
        rv = client.post("/form", headers=headers, json=get_form_request_payload_private())
        assert rv.status_code == 201
        form_id = rv.json.get("formId")
        response = client.post("/public/application/create",json=get_application_create_payload(form_id))
    ...
    def test_application_inactive_post(self,client):
        ...
        rv = client.post("/form", headers=headers, json=get_form_request_payload_public_inactive())
        assert rv.status_code == 201
        form_id = rv.json.get("formId")
        response = client.post("/public/application/create",json=get_application_create_payload(form_id))
        assert response.status_code == 401
