...
def sentiment_entity_analysis_old(  # pylint: disable=too-many-locals
    text: str, topics: List[str] = None
) -> Dict:
...
def load_model_output(text: str):
    """Function to load the trained machine learning model for inference.

    It return the output as the necessary entities and topics for each element.
...
def get_entities_mapper(labels: List[str], sentences: List[str]):
    """Function to get the entities and their sentiment from the model output."""
    entity_text_mapper = {}
...
def get_sentiment_mapper(entity_text_mapper: dict, text_input: str):
    """Function to get the sentiment of the entities from the model output."""
    sentiment_mapper = {}
