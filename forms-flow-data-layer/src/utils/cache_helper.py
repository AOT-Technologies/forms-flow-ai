...
def generate_cache_key(func: Callable, kwargs: dict, key_prefix: Optional[str]) -> str:
    ...
    param_hash = hashlib.sha256(orjson.dumps(query_params, option=orjson.OPT_SORT_KEYS)).hexdigest()
...
def deserialize_result(result_type: Any, cached_data: str):
    ...
    if isinstance(data_dict, list) and result_type:
        ...
        return [item_type(**item) for item in data_dict] if is_dataclass(item_type) else data_dict
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                if cached_result := await redis_cache.get_cache(cache_key):
                    return deserialize_result(func.__annotations__.get("return"), cached_result)
