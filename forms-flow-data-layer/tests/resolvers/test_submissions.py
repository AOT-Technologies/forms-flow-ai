...
async def test_get_submissions_with_data(
    async_webapi_db, async_bpm_db, async_formio_db
):
    """
    ...
    try:
        async with session_api as api_session, session_bpm as bpm_session:
            ...
            submission_insert_result = await async_formio_db["submissions"].insert_one(
                submission_data
            )
        ...
        async with session_api as api_session, session_bpm as bpm_session:
            if submission_id:
                await api_session.execute(
                    delete(application_table).where(
                        application_table.c.submission_id == submission_id
                    )
                )
            if form_process_mapper_id:
                await api_session.execute(
                    delete(mapper_table).where(
                        mapper_table.c.id == form_process_mapper_id
                    )
                )
            await api_session.commit()
            ...
            await bpm_session.execute(
                delete(process_instance_table).where(
                    process_instance_table.c.id_ == proc_inst_id
                )
            )
            await bpm_session.execute(
                delete(process_def_table).where(
                    process_def_table.c.id_ == process_def_id
                )
            )
            await bpm_session.commit()
        # Delete only the created form and submission data from MongoDB
        if submission_id:
            await async_formio_db["submissions"].delete_one(
...
@pytest.mark.asyncio
async def test_get_submissions_with_empty_response(
    async_webapi_db, async_bpm_db, async_formio_db
):
    """
