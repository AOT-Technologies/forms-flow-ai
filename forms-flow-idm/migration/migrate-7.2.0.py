...
import argparse
from utils import get_access_token, get_client_id, add_client_roles, update_client_role, role_changes_710, get_group_id_by_path, get_client_roles, assign_roles_to_group, get_groups, get_sub_groups
...
def _migrate_default_groups_multitenant(token, client_id, client_roles, tenant):
    """Migrate new roles to default groups in multi-tenant mode."""
    try:
        ...
        for group, roles in default_groups_to_role_mappings.items():
            group_id = get_group_id_by_path(token, group)
            if not group_id:
                print(f"Group {group} not found, skipping and continuing the migration.")
                continue
            roles_to_assign = [role for role in client_roles if role['name'] in roles]
            assign_roles_to_group(token, group_id, client_id, roles_to_assign)
    except Exception as e:
        print(f"Error: {e}")

def _migrate_custom_groups_multitenant(token, client_id, client_roles, tenant):
    """Migrate new roles to custom groups in multi-tenant mode."""

    groups = get_groups(token)
    tenant_groups = [
        group for group in groups if group["path"].startswith(f"/{tenant}")
    ]
    exclude_groups = [
        f"/{tenant}-admin",
        f"/{tenant}-formsflow-reviewer",
    ...
    process_groups_assign_roles(tenant_groups, exclude_groups, role_index, token, client_id, tenant)
    print(f"Custom groups migration completed for tenant: {tenant}")
...
def process_groups_assign_roles(groups, exclude_groups, role_index, token, client_id, tenant=None):
    ...
    for group in groups:
        ...
        if tenant:
            # For multi-tenant, use tenant-specific client roles
            group_roles = group.get("clientRoles").get(f"{tenant}-forms-flow-web")
        else:
            # For single-tenant, use the default client roles
            group_roles = group.get("clientRoles").get("forms-flow-web")
        if not group_roles:
            continue  # Skip if no client roles
        print(f"Custom Group: {group.get('path')}")
        ...
        if group.get("subGroupCount", 0) > 0:
            ...
            process_groups_assign_roles(sub_groups, exclude_groups, role_index, token, client_id, tenant)
    return True
