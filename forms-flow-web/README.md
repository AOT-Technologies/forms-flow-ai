...
### Installation
 ...
 `CLIENT_ROLE_ID`|form.io client role Id|eg. 10121d8f7fadb18402a4c|`must get the value from forms-flow-forms resource ` **http://{your-ip-addres}:3001/role**
 `REVIEWER_ROLE`|The role name used for staff/reviewer users||`formsflow-reviewer`
 `REVIEWER_ROLE_ID`|form.io reviewer role Id|eg. 5ee10121d8f7fa03b3402a4d|`must get the value from forms-flow-forms resource ` **http://{your-ip-addres}:3001/role**
 `DESIGNER_ROLE`|The role name used for designer users||`formsflow-designer`
 `DESIGNER_ROLE_ID`|form.io administrator role Id|eg. 5ee090afee045f1597609cae|`must get the value from forms-flow-forms resource ` **http://{your-ip-addres}:3001/role**
 `ANONYMOUS_ID`|form.io anonymous role Id|eg. 5ee090b0ee045f28ad609cb0|`must get the value from forms-flow-forms resource ` **http://{your-ip-addres}:3001/role**
 `USER_RESOURCE_ID`|User forms form-Id|eg. 5ee090b0ee045f51c5609cb1|`must get the value from forms-flow-forms resource `**http://{your-ip-addres}:3001/user**
 `FORMIO_DEFAULT_PROJECT_URL`|The URL of the form.io server||`http://{your-ip-addres}:3001`
 `INSIGHT_API_URL`|Insight Api base end-point||`http://{your-ip-addres}:7000`
 `INSIGHT_API_KEY`|API_KEY from REDASH|eg. G6ozrFn15l5YJkpHcMZaKOlAhYZxFPhJl5Xr7vQw| must be set to your ReDash API key
 `FORMSFLOW_API_URL`|formsflow Rest API URI||`http://{your-ip-addres}:5000/api`
 `CAMUNDA_API_URL`|Camunda Rest API URI||`http://{your-ip-addres}:8000/camunda`
 `KEYCLOAK_URL`| URL to your Keycloak server || `http://{your-ip-addres}:8080`
