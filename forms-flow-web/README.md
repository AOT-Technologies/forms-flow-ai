...
### Installation
 ...
 `CLIENT_ROLE_ID`|form.io client role Id|eg. 10121d8f7fadb18402a4c|`must get the value from forms-flow-forms resource ` **http://host.docker.internal:3001/role**
 `REVIEWER_ROLE`|The role name used for staff/reviewer users||`formsflow-reviewer`
 `REVIEWER_ROLE_ID`|form.io reviewer role Id|eg. 5ee10121d8f7fa03b3402a4d|`must get the value from forms-flow-forms resource ` **http://host.docker.internal:3001/role**
 `DESIGNER_ROLE`|The role name used for designer users||`formsflow-designer`
 `DESIGNER_ROLE_ID`|form.io administrator role Id|eg. 5ee090afee045f1597609cae|`must get the value from forms-flow-forms resource ` **http://host.docker.internal:3001/role**
 `ANONYMOUS_ID`|form.io anonymous role Id|eg. 5ee090b0ee045f28ad609cb0|`must get the value from forms-flow-forms resource ` **http://host.docker.internal:3001/role**
 `USER_RESOURCE_ID`|User forms form-Id|eg. 5ee090b0ee045f51c5609cb1|`must get the value from forms-flow-forms resource `**http://host.docker.internal:3001/user**
 `FORMIO_DEFAULT_PROJECT_URL`|The URL of the form.io server||`http://host.docker.internal:3001`
 ...
 `FORMSFLOW_API_URL`|formsflow Rest API URI||`http://host.docker.internal:5000/api`
...
### Health Check
   ...
   * The application should be up and available for use at port defaulted to 3000 in  http://host.docker.internal:3000/
   * Default user credentials are provided [here](../forms-flow-idm/keycloak/README.md#health-check).

### How to Create Your First Form
  * Login to **http://host.docker.internal:3000/** using valid [designer](../forms-flow-idm/keycloak/README.md#default-user-credentials) credentials 
