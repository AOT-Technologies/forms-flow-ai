...
export const updateGroup = (data) => {
  const apiUpdateGroup = replaceUrl(API.UPDATE_GROUPS, "<groupId>", data.group);
  return (dispatch) => {
...
export const getCleanedDashboards = (dashboards) => {
  ...
  for (let str of dashboards) {
    ...
    // to identify possible object patterns and to extract the key and 
    //value splits the string based on ":" seperator
    let substrArray = substr.split(":");
    let newObj = {};
    let id = null;

    // The data given by the api seems to have consistent patterns which are essential for
    // a non fragile implmentation. All the entries after the first entry in the string 
    //representation of the array have
