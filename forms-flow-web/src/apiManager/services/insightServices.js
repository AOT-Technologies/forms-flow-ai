...
import { serviceActionError } from "../../actions/taskApplicationHistoryActions";
import {
  getDashboards,
  getDashboardDetail,
...
// To keep track of the Indexes that were added to pFrevent duplication
let addedIdxs = [];
...
export const fetchDashboardsList = (dashboardsFromRedash) => {
  return (dispatch) => {
    ...
    for (let dashboard of dashboards) {
      let entry = fetchCleanedDashboardsFromLocalStorage(
        dashboard,
...
export const fetchCleanedDashboardsFromLocalStorage = (
  dashboards,
  dashboardsFromRedash
) => {
  ...
  for (let dashboard of dashboardsArray) {
    ...
    // to identify possible object patterns and to extract the key and value 
    //splits the string based on ":" seperator
    let item = dashboard.split(":");
    let id = null;
    // The data given by the api seems to have consistent patterns which are essential for
    // a non fragile implmentation. All the entries after the first entry in 
    //the string representation of the array have
