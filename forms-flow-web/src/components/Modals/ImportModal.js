...
const ImportModal = React.memo(({ importModal, onClose,
    uploadActionType, importError, importLoader, formName, description,
    handleImport, fileItems, fileType, primaryButtonText, headerText, processVersion }) => {
    ...
    useEffect(() => {
        // Clear selected file and states when the modal closes
        if (!importModal) {
            closeModal();
        }
    }, [importModal]);
    ...
    return (
        <Modal show={importModal} onHide={closeModal} centered={true} size="sm">
            <Modal.Header>
                <Modal.Title>
                ...
                {selectedFile ? (<>
                    ...
                    <div className="upload-boady">
                        <div className="upload-details">
                            ...
                            <span className={`${!importLoader && !importError && !inprogress ? 'upload-status-success' : !importLoader && importError && !inprogress ? 'upload-status-error' : inprogress ? 'upload-status-progress' : null}`}>
                                {
                        ...
                        <div>
                            {importError && !importError.includes("already exists")
                                && <span className="upload-status-error"><Translation>{(t) =>
                                    t("A system error occurred during import. Please try again to import.")}
                                </Translation></span>}
                        </div>
                    </div>
            ...
            <Modal.Footer>
                <CustomButton
                    ...
                    buttonLoading={!importError && importLoader ? true : false}
                    className=""
