          ...
          {isAuthenticated ?
            <Navbar.Collapse id="responsive-navbar-nav" className="navbar-nav">
              <Nav id="main-menu-nav" className="mr-auto active align-items-lg-center">
                <Nav.Link as={Link} to={`${baseUrl}form`} className={`main-nav nav-item ${pathname.match(/^\/form/) ? "active-tab" : ""
                  }`}><i className="fa fa-wpforms fa-fw fa-lg mr-2" />{t("Forms")}</Nav.Link>
                {(getUserRolePermission(userRoles, STAFF_DESIGNER)) ?
                  (<Nav.Link as={Link} to={`${baseUrl}admin`} className={`main-nav nav-item ${pathname.match(/^\/admin/) ? "active-tab" : ""
                ...
                {showApplications ? (getUserRolePermission(userRoles, STAFF_REVIEWER) || getUserRolePermission(userRoles, CLIENT)) ?
                  (<Nav.Link as={Link} to={`${baseUrl}application`} className={`main-nav nav-item ${pathname.match(/^\/application/) ? "active-tab" : ""
                ...
                {getUserRolePermission(userRoles, STAFF_REVIEWER) ? <NavDropdown data-testid="Dashboards" title={<><i className="fa fa-tachometer fa-lg fa-fw mr-2" />{t("Dashboards")}</>}
                  ...
                  className={`main-nav nav-item ${pathname.match(/^\/metrics/) || pathname.match(/^\/insights/) ? "active-tab-dropdown" : ""
                    }`}>
                  <NavDropdown.Item as={Link} to={`${baseUrl}metrics`} className={`main-nav nav-item ${pathname.match(/^\/metrics/) ? "active-tab" : ""
                    }`}><i className="fa fa-pie-chart fa-fw fa-lg" />{t("Metrics")}</NavDropdown.Item>
                  {getUserInsightsPermission() && <NavDropdown.Item as={Link} to={`${baseUrl}insights`} className={`main-nav nav-item ${pathname.match(/^\/insights/) ? "active-tab" : ""
              ...
              <Nav className="ml-lg-auto mr-auto px-lg-0 px-3">
                <Dropdown alignRight>
                  ...
                  <Dropdown.Menu >
                    {
                      selectLanguages.map(e => <Dropdown.Item onClick={() => { handleOnclick(e.name) }}>{e.value}  </Dropdown.Item>)
                    }
