...
const bpmForms = (state = initialState, action) => {
  switch (action.type) {
    case ACTION_CONSTANTS.BPM_FORM_LIST:
      return {
        ...state,
        forms: getPaginatedForms(
          formatForms(action.payload.forms),
          state.limit,
          state.maintainPagination ? state.pagination.page : 1,
          state.sort
        ),
    ...
    case ACTION_CONSTANTS.BPM_FORM_LIST_PAGE_CHANGE:
      return {
        ...
        forms: state.searchText
          ? getPaginatedForms(
              state.formSearchList,
              state.limit,
              action.payload,
              state.sort
            )
    ...
    case ACTION_CONSTANTS.BPM_FORM_LIST_LIMIT_CHANGE:{
      const totalFormsLength = state.searchText
        ? state.formSearchList.length
        : state.formsFullList.length;
      return {
        ...state,
      ...
      return {
        ...
        forms: state.searchText
          ? getPaginatedForms(
              state.formSearchList,
              state.limit,
              state.maintainPagination ? state.page : 1,
              action.payload
            )
          : getPaginatedForms(
              state.formsFullList,
