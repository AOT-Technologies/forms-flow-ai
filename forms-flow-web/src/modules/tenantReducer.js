...
const initialState = {
  ...
  tenantDetail: sessionStorage.getItem('tenant')? JSON.parse(sessionStorage.getItem('tenant')) : null,
...
const tenants =  (state = initialState, action) => {
  switch (action.type) {
    ...
    case ACTION_CONSTANTS.SET_TENANT_DETAILS:
      ...
      return {...state, tenantDetail:action.payload, isTenantDetailLoading:false};
    ...
    case ACTION_CONSTANTS.RESET_TENANT:
      ...
      return {...state, tenantId:action.payload, tenantDetail:null, tenantName: ""};
    default:
      return state;
  }
};
