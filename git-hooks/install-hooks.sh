...
TEMPLATE_FILE="$SCRIPT_DIR/commit-template.txt"
...
chmod +x "$HOOKS_DIR"/* >/dev/null 2>&1

# Copy commit template
cp "$TEMPLATE_FILE" "$HOOKS_DIR"/ >/dev/null 2>&1

# Create the git icommit alias globally
echo "ℹ️  Setting up 'git icommit' alias..."
...
git config --global alias.icommit "!sh \"$INTERACTIVE_COMMIT_SCRIPT_GIT_PATH\""
if [ $? -eq 0 ]; then
    echo "✅ 'git icommit' alias created successfully!"
else
    echo "⚠️  Warning: Failed to create 'git icommit' alias. You may need to create it manually:"
    echo "   git config --global alias.icommit '!sh \"$INTERACTIVE_COMMIT_SCRIPT_GIT_PATH\"'"
fi
